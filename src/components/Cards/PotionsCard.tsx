...
import { DEFAULT_POTION_URL } from "../../utils/constants.tsx";

const MovieCard = ({ potData }) => {
  const { slug, image, effect, characteristics } = potData.attributes;
  return (
    <div className="w-60 m-10 rounded-xl cursor-pointer shadow-md h-96 overflow-y-hidden ">
      <img
        className="rounded-xl p-2 h-52 w-56"
        src={image === null ? DEFAULT_POTION_URL : image}
      />
